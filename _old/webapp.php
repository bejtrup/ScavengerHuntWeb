...
//        $uploader->upload($_FILES['file']['tmp_name'], $_POST['destination'], $_FILES['file']['name']);
       // $newName = $_POST['teamNum'] . $_FILES["file"]["type"]
       $uploader->upload($_FILES['file']['tmp_name'], $_POST['destination'], $_FILES['file']['name']);
...
<form method="POST" enctype="multipart/form-data">
    <dl>
        <dt><label for="teamNum">Holdnavn</label></dt>
        <dd><input type="text" id="teamNum" name="teamNum" value="Hold01"></dd>
        <dt><label for="email">Dropbox e-mail</label></dt>
        <dd><input type="text" id="email" name="email" value="bejtrup@gmail.com"></dd>
        <dt><label for="password">Dropbox password</label></dt>
        <dd><input type="password" id="password" name="password" value="david0"></dd>
        <dt><label for="destination">Destination directory (optional)</label></dt>
        <dd><input type="text" id="destination" name="destination" value="TESTsh"> e.g. "dir/subdirectory", will be created if it
            doesn't exist
        </dd>
