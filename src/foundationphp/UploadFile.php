...
class UploadFile
{
	...
	protected function checkName($file)
	{
		...
		//$nospaces = str_replace(' ', '_', $file['name']);
                ...
                $fileNewName = $_POST['TaskName'] . "." . $ext; 
                //$fileNewName = $file['name'];
                $nospaces = str_replace(' ', '_', $fileNewName);
                
		if ($nospaces != $fileNewName) {
		...
		if ($this->renameDuplicates) {
			$name = isset($this->newName) ? $this->newName : $fileNewName;
			$existing = scandir($this->destination);
			if (in_array($name, $existing)) {
				$i = 1;
				do {
					//$this->newName = $nameparts['filename'] . '_' . $i++;
                                        $this->newName = $fileNewName . '_' . $i++;
					if (!empty($extension)) {
	...
	protected function moveFile($file)
	{
                ...
                $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
                $fileNewName = $_POST['TaskName'] . "." . $ext; 
		$filename = isset($this->newName) ? $this->newName : $fileNewName;

                //$ext = pathinfo($file['name'], PATHINFO_EXTENSION);
                //$filename = $_POST['TaskName'] . "." . $ext; 
		...
		if ($success) {
			...
			$this->messages[] = $result;
                         header('Location: webapp.php');  // for at geninlæse siden..
		} else {
